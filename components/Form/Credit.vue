...
let marks = useSiteConfig().catalog;
marks = marks.sort((a: any, b: any) => a.title.toLowerCase().localeCompare(b.title.toLowerCase()))
let mark = ref<CatalogType | null>()
let folders = computed(() => {
  return mark.value?.folders.sort((a: any, b: any) => a.title.toLowerCase().localeCompare(b.title.toLowerCase())) || []
})
